...
Open [process.env.BACKEND_URL](process.env.BACKEND_URL) to view it in the browser.
...
In the WebStorm menu `Run` select `Edit Configurations...`. Then click `+` and select `JavaScript Debug`. Paste `process.env.BACKEND_URL` into the URL field and save the configuration.
...
This way, when you `fetch('/api/todos')` in development, the development server will recognize that it’s not a static asset, and will proxy your request to `process.env.BACKEND_URL/api/todos` as a fallback. The development server will **only** attempt to send requests without `text/html` in its `Accept` header to the proxy.

Conveniently, this avoids [CORS issues](http://stackoverflow.com/questions/21854516/understanding-ajax-cors-and-security-considerations) and error messages like this in development:

```
Fetch API cannot load process.env.BACKEND_URL/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'process.env.BACKEND_URL' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
